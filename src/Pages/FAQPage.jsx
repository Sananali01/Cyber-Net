    ...
    <div className="min-h-screen py-12 text-white ">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <HelpCircle className="inline-block h-12 w-12 text-red-600 mb-4" />
        ...
        <div className="mt-8">
          {faqs.map((faq, index) => (
            <div key={index} className="bg-gray-800 rounded-lg mb-4">
              <div
                className={`p-6 cursor-pointer ${activeIndex === index ? 'bg-gray-700' : 'hover:bg-gray-700'}`}
                onClick={() => toggleAccordion(index)}
              >
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium text-grey-100">{faq.question}</h3>
                  <svg
                    className={`h-6 w-6 transform ${activeIndex === index ? 'rotate-180 text-yellow-400' : 'rotate-0 text-gray-400'} transition-transform`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
